...
void main(int argc, char **argv) {
     ...
     * 调用SSL_CTX_new创建本地上下文，记录握手参数及与SSL连接有关的状态
     * ========================= */
    SSLeay_add_ssl_algorithms();
    client_method = SSLv2_client_method();
    SSL_load_error_strings();
    ctx = SSL_CTX_new(client_method);
    printf("(1) SSL context initialize\n\n");

    /* ===========================================
     * (2) convert server hostname into IP address
     * Unix的gethostbyname函数将主机名转换成IP
     * =========================================== */
    hostname = argv[1];
     ...
     * 创建本地套接字， 设置远端地址信息并连接到远端服务器，建立到服务器端口443的TCP连接
     * =============================================== */
    ...
    memcopy(&(server_socket_address.sin_addr.s_addr),
    ...
    /* ========================================================
     * (4) initialize the SSL handshake over the TCP connection
     * 
     * ======================================================== */
    ssl = SSL_new(ctx); /* create SSL stack endpoint */
    SSL_set_fd(ssl, sd); /* attach SSL stack to socket */
    err = SSL_connect(ssl); /* initialize SSL handshake */
     ...
     * ===================================================== */
    sprintf(host_addr, "Host: %s:443\r\n", hostname);
    strcopy(outbuf, "GET / HTTP/1.0\r\n");
    ...
    /* ===============================
     * (9) close connection & clean up
     * =============================== */
    SSL_shutdown(ssl);
    close(sd);
    SSL_free(ssl);
    SSL_CTX_free("(9) all done, cleaned up and closed connection\n\n");
}
